...
from spot_photo.ml_logic.data import load_X_pred, make_corpus, encode_X_pred, load_pickle,\
     preprocess_img_emmb, preprocess_tensor
...
app.state.X_pred_1 = load_pickle(file_name = 'all_dataset_embedded_allmpnetbasev2.pkl')
app.state.X_pred_2 = load_pickle(file_name='all_dataset_clipvitb32.pkl')

app.state.corpus_X_pred_1 = make_corpus(app.state.X_pred_1)

app.state.corpus_X_pred_2 = make_corpus(app.state.X_pred_2)

app.state.X_pred_1_embeddings = preprocess_tensor (app.state.corpus_X_pred_1)
app.state.X_pred_2_embeddings = preprocess_img_emmb (app.state.corpus_X_pred_2)
...
def recherche(model_choice : str, query: str, k: int):
    if model_choice == 'all-mpnet-base-v2':
        ...
        images_index = compute_similarity(query_embedding, app.state.X_pred_1_embeddings, k=k)
    ...
    if model_choice == 'clip-ViT-B-32':
        ...
        query_embedding = embedding_query(model, query)
        images_index = compute_similarity(query_embedding, app.state.X_pred_2_embeddings, k=k)
